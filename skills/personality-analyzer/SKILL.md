...
- 分析対象は **現在のチャット内で確認できるユーザー発言のみ**。過去チャット、記憶、プロフィール、一般的な職業イメージ、外部情報は根拠にしない。
- システム/開発者指示、AGENTS.md、メモリ文脈、ツール出力、引用された第三者発言は、たとえチャット内に表示されていても性格根拠にしない。
- ユーザーが貼った作業指示、分析対象の範囲指定、出力形式指定、言語希望（例: 「英語で出力して」）、最後の診断依頼文（例: 「性格診断して」「analyze my personality lang:en」）、`lang:xx` 指定は、タスク制御には使うが性格特性の根拠にはしない。
- 言語希望や出力形式指定だけの発言は `chat.summary` に含めてよいが、HEXACO / MBTI / Pokemon の reasoning には性格根拠として使わない。
- ただし、同じ発言内にユーザー自身の好み・価値観・感情・対人姿勢が明示されている場合は、その部分だけを根拠にしてよい（例: 「曖昧な説明は苦手。英語で出力して」では前半のみ根拠）。
- 直前までの発言に「この会話では次の発言だけを対象にして」など明示的な対象範囲がある場合は、その範囲を優先する。
- 根拠にした発言が少ない、または技術質問・事務連絡に偏る場合は、credibility を低くし、各 reasoning と general_evaluation で「根拠が薄い仮判定」であることを明示する。技術トピックそのものから職業・能力・性格を一般化しない。

## 分析プロセス

1. **発言収集**: 「分析対象の境界」に従って、根拠に使えるユーザー発言を抽出
...
- **chat.title / chat.summary**: 性格根拠として使える発言を中心に要約する。根拠対象外の制御発言（言語指定、出力形式指定、貼り付け文脈など）は、会話理解に必要な場合だけ最小限に触れる。
...
MBTI は `type` と reasoning のみを出力し、軸ごとの数値スコアは追加しない。根拠が薄い軸は reasoning 内で暫定判定であることを明示する。
...
- credibility が 25 以下の場合は、多くの HEXACO 因子を 3.0 付近に置き、MBTI とポケモン選出も低信頼度の推測として書く
- ポケモン選出は固定対応表ではなく、HEXACOプロファイルとの性格的類似性で判断する（ゲーム内能力ではなく、性格・イメージ面で）。reasoning では、どの因子または性格傾向に対応する選出かを明示する。低 credibility では「低信頼度の推測」であることも各 reasoning に含める
- credibility が低くても `Pokemon` は必ず 3 件出す。根拠不足を理由に省略せず、各 reasoning で推測性を明示する
